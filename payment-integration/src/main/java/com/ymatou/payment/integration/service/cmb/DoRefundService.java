 ...
 */
@Component
public class DoRefundService implements InitializingBean {
    private CloseableHttpClient httpClient;

    @Autowired
    private IntegrationConfig integrationConfig;

    public CmbDoRefundResponse doService(CmbDoRefundRequest request, HashMap<String, String> header)
            throws Exception {
        String url = integrationConfig.getCmbDoRefundUrl(header);
        List<NameValuePair> body = getRequestBody(request);
        String resp = HttpClientUtil.sendPost(url, body, header, httpClient);

        CmbDoRefundResponse response = JSON.parseObject(resp, CmbDoRefundResponse.class);

        return response;
    }

    private List<NameValuePair> getRequestBody(CmbDoRefundRequest req) {
