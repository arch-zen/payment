 ...
 */
@Component
public class QuerySingleOrderService implements InitializingBean {
    private CloseableHttpClient httpClient;

    @Autowired
    private IntegrationConfig integrationConfig;

    public CmbQuerySingleOrderResponse doService(CmbQuerySingleOrderRequest request, HashMap<String, String> header)
            throws Exception {
        String url = integrationConfig.getCmbQuerySingleOrderUrl(header);
        List<NameValuePair> body = getRequestBody(request);
        String resp = HttpClientUtil.sendPost(url, body, header, httpClient);

        CmbQuerySingleOrderResponse response = JSON.parseObject(resp, CmbQuerySingleOrderResponse.class);

        return response;
    }

    private List<NameValuePair> getRequestBody(CmbQuerySingleOrderRequest req) {
