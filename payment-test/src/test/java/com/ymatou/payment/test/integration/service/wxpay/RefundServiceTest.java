...
public class RefundServiceTest extends RestBaseTest {
    ...
    public void testDoServiceWithMock() throws Exception {
        ...
        mockHeader.put("MockResult-WeiXin-return_code", "SUCCESS");
        mockHeader.put("MockResult-WeiXin-result_code", "ERROR");

        WxRefundResponse response = refundService.doService(request, mockHeader);


        Assert.assertEquals("SUCCESS", response.getReturn_code());
        Assert.assertEquals("ERROR", response.getResult_code());
    }
